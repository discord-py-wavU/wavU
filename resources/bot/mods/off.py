...
class OffCommand(commands.Cog, Helpers):
    ...
    async def off(self, ctx, arg=None):
        ...
        if audios:
            ...
            msg = "Choose a number to disabled a _**.mp3**_ file, _**cancel**_ or _**all**_\n"
            ...
            try:
                for i in range(3):
                    ...
                    if msg.content.isdigit() and int(msg.content) <= len(audios) and int(msg.content) != 0:
                        ...
                        await self.disable_audio(obj, hashcode)
                        await self.embed_msg(ctx, f"Thanks {ctx.message.author.name} for using wavU :wave:",
                                             f'**{audios[int(msg.content) - 1]}** has been _**disabled**_', 30)
                        break
                    elif str(msg.content).lower() == "cancel":
                        await self.embed_msg(ctx, f"Thanks {ctx.message.author.name} for using wavU :wave:",
                                             "Nothing has been _**disabled**_", 30)
                        break
                        ...
                        for index in range(len(audios)):
                            ...
                            await self.disable_audio(obj, hashcode)
                        await self.embed_msg(ctx, f"Thanks {ctx.message.author.name} for using wavU :wave:",
                                             "All the _**.mp3**_ files has been _**disabled**_", 30)
                        break
                        ...
                        if i == 2:
                            await self.embed_msg(ctx, f"I'm sorry, {ctx.message.author.name} :cry:",
                                                 "None of the attempts were correct, _**disabled**_ has been aborted",
                                                 10)
...
def setup(client):
    client.add_cog(OffCommand(client))
